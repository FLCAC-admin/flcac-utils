...
def _get_object(owner, repo, object_type, refId, token):
    url = f"{commons_base}/ws/public/browse/{owner}/{repo}/{object_type}/{refId}"
    cookies = {"JSESSIONID": token}
    headers = {"Accept": "application/json", "Content-Type": "application/json"}
...
def return_request(owner, repo, object_type="PROCESS", **kwargs):
    ...
    url = (
        f"{commons_base}/ws/public/download/json/prepare/"
    ...
    download_url = (
        "https://www.lcacommons.gov/lca-collaboration/ws/public/download/json"
    )
            ...
            if "PROCESS" in object_types:
                if name.startswith("process"):
                    ...
                    if check_obj_append(d, search_objs, obj_type="PROCESS"):
                        parsed = olca.Process.from_dict(d)
            ...
            if "IMPACT_METHOD" in object_types:
                if name.startswith("lcia_categories"):
                    ...
                    if check_obj_append(d, search_objs, obj_type="IMPACT_METHOD"):
                        parsed = olca.ImpactCategory.from_dict(d)
                        object_list.append(parsed)
                    else:
                        continue
                    ...
                    d = read_json(f, name)
                    if check_obj_append(d, search_objs, obj_type="IMPACT_METHOD"):
                        parsed = olca.ImpactMethod.from_dict(d)
                        object_list.append(parsed)
                    else:
                        continue
            ...
            if "SOURCES" in object_types:
                if name.startswith("source"):
                    ...
                    if check_obj_append(d, search_objs, obj_type="SOURCES"):
                        parsed = olca.Source.from_dict(d)
                        object_list.append(parsed)
                    else:
                        continue
            if "DQ_SYSTEM" in object_types:
                if name.startswith("dq_system"):
                    ...
                    if check_obj_append(d, search_objs, obj_type="DQ_SYSTEM"):
                        parsed = olca.DQSystem.from_dict(d)
                        object_list.append(parsed)
                    else:
                        continue
...
def read_commons_data(object_dict, auth=False):
    ...
    for repo, object_types in object_dict.items():
        ...
        api_objects = [
            i for i in object_types if i in ("PROCESS", "DQ_SYSTEM", "IMPACT_METHOD")
        ]
        ...
        data_dict[repo] = process_response(
            resp, object_types=object_types, search_objs=search_objs
        )
    return data_dict
...
if __name__ == "__main__":
    object_dict = {
        ...
        #     {'ACTORS': ['NETL', 'NREL'],
        #      'PROCESS': [
        #          'coal extraction and processing - Northern Appalachia,',
        #          'BIT, Underground',
        #      ]},
    ...
    flow = get_single_object(
        "Forestry and Forest Products", "FLOW", "209abd9b-df20-39e2-a366-63b360e896bf"
    )
