...
def prepare_tech_flow_mappings(df, auth=False):
    ...
    flow_dict = {
        row["SourceFlowName"]: {
            ...
            "bridge_flow_name": row["BridgeFlowName"]
            if row.get("BridgeFlowName")
            else np.nan,
            "name": row["TargetFlowName"],
            "provider": row.get("Provider") if not row.get("Bridge") else np.nan,
            "repo": np.nan
            if pd.isna(row["TargetRepoName"])
            else {row["TargetRepoName"]: row["TargetFlowName"]},
...
def apply_tech_flow_mapping(
    df, flow_dict, flow_objs, provider_dict, cond=None
) -> pd.DataFrame:
    """
    ...
    df = (
        ...
        .assign(
            bridge=lambda x: np.where(
                cond,
                x["name"].map(
                    {k: True for k, v in flow_dict.items() if v.get("BRIDGE", False)}
                ),
                False,
            )
        )
        .assign(
            repo=lambda x: np.where(
                cond,
                x["name"].map(
                    {
                        k: list(v["repo"].keys())[0]
                        for k, v in flow_dict.items()
        ...
        .assign(
            bridge_flow_name=lambda x: np.where(
                cond,
                x["name"].map(
                    {
                        k: v["bridge_flow_name"]
                        for k, v in flow_dict.items()
                        if v.get("BRIDGE", False)
                    }
                ),
    ...
    df = (
        ...
        .assign(
            FlowUUID=lambda x: np.where(
                cond,
                x["name"]
                .map({k: v["id"] for k, v in flow_dict.items() if "id" in v})
                .fillna(x["FlowUUID"]),
                x["FlowUUID"],
            )
        )
        .assign(
            FlowName=lambda x: np.where(
                cond,
                x["name"]
                .map({k: v["name"] for k, v in flow_dict.items()})
                .fillna(x["name"]),
                x["name"],
            )
        )
        .assign(
            Context=lambda x: np.where(
                cond,
                x["name"]
                .map({k: get_context(v["name"]) for k, v in flow_dict.items()})
                .fillna(x["Context"]),
                x["Context"],
            )
        )
    ...
    df = (
        ...
        .assign(
            unit=lambda x: np.where(
                cond2,
                x["name"]
                .map({k: v.get("unit") for k, v in flow_dict.items()})
                .fillna(x["unit"]),
                x["unit"],
            )
        )
        .assign(
            conversion=lambda x: np.where(
                cond2,
                x["name"]
                .map({k: v.get("conversion", 1) for k, v in flow_dict.items()})
                .fillna(1),
                1,
            )
        )
...
def create_bridge_processes(df, flow_dict, flow_objs):
    ...
    if len(df.query("bridge == True")) == 0:
        return pd.DataFrame()
    df_bridge = (
        df.query("bridge == True")
        .drop_duplicates(subset="FlowName")
        .drop(columns=["default_provider", "default_provider_process"], errors="ignore")
        .reset_index(drop=True)
        .assign(amount=1)
        .assign(
            ProcessCategory=lambda x: x.apply(
                lambda z: create_bridge_category(z["repo"], z["bridge_flow_name"]),
                axis=1,
            )
        )
        .assign(
            ProcessName=lambda x: x.apply(
